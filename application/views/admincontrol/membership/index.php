...
<div class="plugin-uploader">
	<p class="text-center text-help">If you have a plugin in a .zip format, you may install it by uploading it here. <br>If you want to create custom payment gateway than follow this <a href="<?= base_url('membership/membership_payment_gateways_doc') ?>">documentation</a></p>
						...
						<tr>
							...
							<td class="text-right">
								<?php if($payment['is_install'] == '1'){ ?>
									<a href="<?= base_url('membership/membership_payment_gateways_edit/'. $payment['code']) ?>" class="btn btn-sm btn-info">Edit</a>
								<?php } ?>
								<a onclick="return confirm('Are you sure?')" href="<?= base_url('membership/membership_payment_gateways_status_change/'. $payment['code']) ?>" class="btn btn-sm btn-<?= $payment['is_install'] == "1" ? "danger" : "success" ?>"><?= $payment['is_install'] == "1" ? "Un-Install" : "Install" ?></a>

								<a onclick="return confirm('Are you sure?')" href="<?= base_url('membership/delete_plugin/'.$payment['code']) ?>" class="btn btn-sm btn-danger">Delete</a>
							</td>
						</tr>
					<?php } ?>
				</tbody>
...
<script type="text/javascript">
	...
	$("#plugin-file-button").on("click", function(evt){
        ...
        $.ajax({
            url:'<?= base_url('membership/installPayementGateway') ?>',
            type:'POST',
            dataType:'json',
            cache:false,
            contentType: false,
