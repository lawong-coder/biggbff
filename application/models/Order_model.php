...
class Order_model extends MY_Model{
    ...
    public function getAllOrders($filter = array(), $addShipping = true){
        ...
        //AND op.refer_id = wallet.user_id
        $union1 = array(
            '"store" as type',
            '"[]" as custom_data',
            '(SELECT status FROM wallet WHERE wallet.reference_id = op.order_id AND comm_from="store" AND type IN ("sale_commission", "vendor_sale_commission")  LIMIT 1) as wallet_status',
            'o.id',
            'o.status',
            'o.user_id',
            'o.total',
    ...
    public function getOrders($filter = array(), $addShipping = true){
        ...
        $query = "
            SELECT o.*,u.firstname,u.id as user_id, u.lastname ,sum(op.total) AS total_sum,
                (SELECT paypal_status FROM orders_history WHERE orders_history.order_id = o.id ORDER BY id DESC LIMIT 1) as last_status
            FROM `order` o 
                LEFT JOIN users u ON u.id = o.user_id   
                LEFT JOIN order_products op ON op.order_id = o.id
