...
class Membership extends MY_Controller {
	...
	public function submit_plan_form($plan_id = 0){
		...
		$this->form_validation->set_rules('belt_size', 'Belt Size', 'required|trim');
		...
		if (!isset($json['errors'])) {
			...
			$plan->label_text = isset($data['label_text']) ? $data['label_text'] : NULL;
		
			$plan->label_background = isset($data['label_background']) ? $data['label_background'] : '#28A745';
		
			$plan->label_color = isset($data['label_color']) ? $data['label_color'] : '#FFFFFF';
			...
			if(isset($data['product']) && !empty($data['product']))
			{
                ...
                $plan->products = json_encode($products);
			}

			$plan->hosting_cost = isset($data['hosting_cost']) ? $data['hosting_cost'] : NULL;
			$plan->fego_shares = isset($data['fego_shares']) ? $data['fego_shares'] : NULL;
			$plan->bigg_coupon = isset($data['bigg_coupon']) ? $data['bigg_coupon'] : NULL;
			$plan->bigg_coins = isset($data['bigg_coins']) ? $data['bigg_coins'] : NULL;
			$plan->other_benefits = isset($data['other_benefits']) ? $data['other_benefits'] : NULL;
	...
	public function buy_membership($plan_id, $code){
