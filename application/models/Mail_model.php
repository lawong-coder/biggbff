...
class Mail_model extends MY_Model{
    ...
    public function send_subscription_buy($plan_id){
        ...
        $body = $this->load->view('mails/subscription_buy_admin',$data, true);
    ...
    private function sendMail($to, $subject, $htmlContent, $is_store = false, $allow_bootstrap = false){
        try {
            ...
            if($to == 'admin'){
                $admin_list = \App\user::where(['type' => 'admin', 'status' => 1])->pluck('email')->toArray();
            ...
            if(!empty($setting['smtp_hostname']) && !empty($setting['smtp_port']) && !empty($setting['smtp_username']) && !empty($setting['smtp_password'])) {
                ...
                if($this->email->send()) {
                    ...
                    $sendThroughPhpMailer = true;
                }
            } else {
                $sendThroughPhpMailer = true;
            }

            /*if(isset($sendThroughPhpMailer)) {
                $from = (!empty($setting['from_email'])) ? $setting['from_email'] : $setting['smtp_username'];
                $fromName = (!empty($setting['from_name'])) ? $setting['from_name'] : $_SERVER['SERVER_NAME'];
    ...
    public function send_bff_member_register_mail($user, $link){

        $template = $this->getTemplateByID(0, 'bff_member_registration');
    ...
    public function send_ewallet_withdrawal_request_mail($user, $withdrawRequest){
        $data = (array)$user;
        // $data['withdraw_amount'] = $withdrawRequest->amount;
        $template = $this->getTemplateByID(0, 'withdrawal_request_from_member');   

        $body = $this->parseTemplate($template['admin_text'],$template['shortcode'],$data);
        if(isset($withdrawRequest->amount)){
            $body = str_replace("[[withdraw_amount]]", c_format($withdrawRequest->amount), $body);
        }
    ...
    public function send_mail_before_monthly_hosting_transaction($transaction, $transaction_date)
    {
        if(isset($transaction->user) && isset($transaction->plan))
        {
            $template = $this->getTemplateByID(0, 'notify_before_monthly_hosting_transaction');

            $data = [
                'firstname' => $transaction->user->firstname,
                'lastname' => $transaction->user->lastname,
                'transaction_date' => dateFormat($transaction_date, 'd-M-y')
            ];

            $body = $this->parseTemplate($template['text'],$template['shortcode'],$data);
            if(isset($transaction->plan->hosting_cost)){
                $body = str_replace("[[hosting_fees]]", c_format($transaction->plan->hosting_cost), $body);
            }
    ...
    /*public function send_member_belt_exited_notification_mail($membership_users)
    {
        foreach ($membership_users as $key => $membership_user) {
            if(isset($membership_user->user) && isset($membership_user->plan))
            {
                $template = $this->getTemplateByID(0, 'member_belt_exited_notification');

                $data = [
                    'firstname' => $membership_user->user->firstname,
                    'lastname' => $membership_user->user->lastname,
                    'plan' => $membership_user->plan->name
                ];

                $body = $this->parseTemplate($template['text'],$template['shortcode'],$data);

                $to = $membership_user->user->email;
                $template_subject = $this->parseTemplate($template['subject'],$template['shortcode'],$data);
    ...
    public function send_member_belt_exited_notification_mail($users, $plan)
    {
        foreach ($users as $key => $user) {
            if(!empty($user) && !empty($plan))
            {
                log_message('error', 'send_member_belt_exited_notification_mail to_send user:' . print_r($user, TRUE));
    ...
    public function send_ewallet_withdrwal_status_change($withdrawRequest){
        
        $user = App\User::find($withdrawRequest->user_id);

        $data = $user->toArray();
        $data['request_id'] = $withdrawRequest->id;
        $data['status'] = strip_tags(new_withdrwal_status($withdrawRequest->status));
        $data['comment'] = $withdrawRequest->remarks;

        $template = $this->getTemplateByID(0,'ewallet_withdrwal_status_change');   
        $body = $this->parseTemplate($template['text'],$template['shortcode'],$data);
        $_subject = $this->parseTemplate($template['subject'],$template['shortcode'],$data);
