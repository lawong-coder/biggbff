								...
								<div class="form-group">
									...
									<div>
										<div class="btn-group mb-1 d-inline-block btn-group-md" role="group" aria-label="Export/Import Users">
											...
											<a class="btn btn-primary" href="<?php echo base_url("admincontrol/addusers") ?>"><?= __('admin.add_user') ?></a>
								...
								<div class="table-responsive b-0" data-pattern="priority-columns">
									<p class="p-2 mb-0 lead user-approvals-filer">
										<?php if($approvals_count['total'] > 0) { ?>
											<a class="badge <?= (!isset($_GET['apr']) || $_GET['apr'] == 'all') ? 'badge-primary' : 'badge-default' ?>" data-apr="all" href="javascript:void(0);">All (<?= $approvals_count['total']; ?>)</a>
										<?php } ?>
										<?php if($approvals_count['pending'] > 0 || $approvals_count['declined'] > 0) { ?>
											<?php if($approvals_count['approved'] > 0) { ?>
												<a class="badge <?= (isset($_GET['apr']) && $_GET['apr'] == 'approved') ? 'badge-primary' : 'badge-default' ?>" data-apr="approved" href="javascript:void(0);">Activated (<?= $approvals_count['approved']; ?>)</a>
											<?php } ?>
											<?php if($approvals_count['pending'] > 0) { ?>
												<a class="badge <?= (isset($_GET['apr']) && $_GET['apr'] == 'pending') ? 'badge-primary' : 'badge-default' ?>" data-apr="pending" href="javascript:void(0);">Pending (<?= $approvals_count['pending'] ?>)</a>
											<?php } ?>
											<?php if($approvals_count['declined'] > 0) { ?>
												<a class="badge <?= (isset($_GET['apr']) && $_GET['apr'] == 'declined') ? 'badge-primary' : 'badge-default' ?>" data-apr="declined" href="javascript:void(0);"> Declined (<?= $approvals_count['declined'] ?>)</a>
											<?php } ?>
										<?php } ?>	
									</p>
									...
									<table id="tech-companies-1" class="table table-hover user-table">
										<thead class="thead-light">
											<tr>
												<th>Id<!-- <input type="checkbox" class="selectall-wallet-checkbox"> --></th>
												<th>First Name</th>
												<th>Last Name</th>
												<th>Username</th>
												<th>Email</th>
	...
	function getPage(page, existingCounts = null) {
		...
		xhr = $.ajax({
			...
			success:function(json){
				...
				if(json['pagination_result'])
				{
					$(".user-table tfoot .pagination_result").html(json['pagination_result']);
				}
	...
	function reloadApprovalFilter(data) {
		...
		if(data['total'] > 0) {
			if($('.user-approvals-filer a[data-apr="all"]').length <= 0) {
				...
				$('.user-approvals-filer a[data-apr="all"]').text('All ('+data['total']+')')
			}
		} else {
		...
		if(data['approved'] > 0) { 
			if($('.user-approvals-filer a[data-apr="approved"]').length <= 0) {
				$('.user-approvals-filer').append('<a class="badge badge-default" data-apr="approved" href="javascript:void(0);">Activated ('+data['total']+')</a>');
			} else {
				$('.user-approvals-filer a[data-apr="approved"]').text('Activated ('+data['approved']+')')
			}
		} else {
		...
		if(data['pending'] > 0) {
			if($('.user-approvals-filer a[data-apr="pending"]').length <= 0) {
				$('.user-approvals-filer').append('<a class="badge badge-default" data-apr="pending" href="javascript:void(0);">Pending ('+data['total']+')</a>');
			} else {
				$('.user-approvals-filer a[data-apr="pending"]').text('Pending ('+data['pending']+')')
			} 
		} else {
		...
		if(data['declined'] > 0) {
			if($('.user-approvals-filer a[data-apr="declined"]').length <= 0) {
				$('.user-approvals-filer').append('<a class="badge badge-default" data-apr="declined" href="javascript:void(0);">Declined ('+data['total']+')</a>');
			} else {
				$('.user-approvals-filer a[data-apr="declined"]').text('Declined ('+data['declined']+')')
			}
		} else {
