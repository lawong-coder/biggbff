...
class Product_model extends MY_Model{
    ...
    function getLanguageHtml($control = 'admincontrol' , $theme = null){
        ...
        if($theme == 'store-default') {
            ...
            foreach ($all as $key => $value) {
                ...
                $_html .='<a class="dropdown-item" href="'. base_url($control."/change_language/".$value['id']) .'"><img src="'. base_url($value['flag']) .'" alt="" height="16"/><span> '. $value['name'] .' </span></a>';
            }
            $html = '<a class="nav-link dropdown-toggle arrow-none waves-effect text-white" data-toggle="dropdown" href="#" role="button"
                    aria-haspopup="false" aria-expanded="false">
                        <span class="'. $extra_class .'">'. $selected['name'] .'</span> <img src="'. base_url($selected['flag']) .'" class="ml-2" height="16" alt=""/>
    ...
    function getPopulerUsers($filter = array()){
        ...
        if(isset($filter['ids_in']) && (int)$filter['ids_in'] > 0){
            $where .= " AND users.id IN (". $filter['ids_in'] . ")";
        }
        ...
        /*$query = '
            SELECT sum(wallet.amount) as all_commition, wallet.user_id, users.type, users.firstname, users.lastname, users.Country, users.email, countries.sortname
            FROM wallet 
        ...
        $query = '
            SELECT sum(user_balance_logs.amount) as all_commition, e_wallet.user_id, users.type, users.firstname, users.lastname, users.Country, users.email, countries.sortname
            FROM e_wallet 
            LEFT JOIN users ON e_wallet.user_id = users.id
            LEFT JOIN countries ON countries.id = users.Country
            LEFT JOIN user_balance_logs ON user_balance_logs.receiver_id = users.id
            WHERE user_balance_logs.type = "referral_commission" AND user_balance_logs.status=1 AND users.type = "user" '.$where.'
            GROUP BY e_wallet.user_id
            ORDER BY all_commition DESC
        '. $limit;
        ...
        if(!empty($results))
        {
            ...
            foreach ($results as $key => $user) {
                $refids = $this->Total_model->getTeamReferals($user['user_id']);
                // $refids = array_merge($refids, [$user['user_id']]);
                // $ref_ids = join("','",$refids);
                
                $results[$key]['total_referrals'] = count($refids);
            }
        }
    ...
    public function getAvatar($image){
        if ($image != '')
            ...
            return base_url('assets/vertical/assets/images/users/default_avatar.jpg');
    }
    ...
    public function getAllinOneNewFormatQuery($filter = array()){
        $_children = [];
        ...
        $query = 'SELECT
                ...
                LEFT JOIN users up ON up.id = users.refid
                WHERE users.type = "admin" AND users.reg_approved=1';

        $admin_result= $this->db->query($query)->row_array();

        // $admin_result= $this->db->query("SELECT id,username,firstname,lastname,avatar,refid FROM users WHERE type='admin'")->row_array();
        $_children[] = array(
            'id'        => $admin_result['id'],
            'parent_id' => 0,
            'name'      => $admin_result['username'],
            'avtar'     => "<img class='user-avtar-tree' src='". $this->getAvatar($admin_result['avatar']) ."'>",
            'username'  => $admin_result['username'],
            'desc'      => $admin_result['plan_name'] . ((!empty($admin_result['plan_name'])) ? ', ' . $admin_result['country_name'] : $admin_result['country_name']),
            'refid'     => $admin_result['refid']
        );
        ...
        $query = 'SELECT
                ...
                LEFT JOIN membership_plans ON membership_plans.id = users.assigned_plan
                LEFT JOIN users up ON up.id = users.refid
                WHERE users.type = "user" AND users.reg_approved=1';
        // $users= $this->db->query("SELECT id,username as name,avatar,refid FROM users WHERE type='user'")->result_array();
        $users= $this->db->query($query)->result_array();
        foreach ($users as $key => $value) {
            $_children[] = array(
                'id'        => $value['id'],
                'parent_id' => $value['refid'] ? $value['refid'] : 1,
                'name'      => $value['username'],
                'avtar'     => "<img class='user-avtar-tree' src='". $this->getAvatar($value['avatar']) ."'>",
                'username'  => $value['username'],
                'desc'      => $value['plan_name'] . ((!empty($value['plan_name'])) ? ', ' . $value['country_name'] : $value['country_name']),
                'refid'     => $value['refid']
            );
        }
    ...
    public function giveClickCommition($product, $user_id, $viewer_id= 0){
        $product_id = $product['product_id'];
        // $totalClick = $this->db->query("SELECT * FROM  product_action WHERE pay_commition = 0 AND user_id = '{$user_id}' AND  product_id = '{$product_id}' ")->result();
    ...
    public function getMyUnderLevelUsers($user_id, $first_time = true, $user_referlevel = 0){
        ...
        foreach ($users as $key => $value) {
            $value['children'] = $this->getMyUnderLevelUsers($value['id'], false, $user_referlevel);
            $children[] = array(
                'user_referlevel'             => $user_referlevel,
                'id'                          => $value['id'],
                'title'                       => $value['title'],
                'email'                       => $value['email'],
                'children'                    => $value['children'],
    ...
    function getDirectrefUsers($user_id = null){
        if ($user_id) {
            ...
            /*$query = 'SELECT
                ...
                LEFT JOIN membership_plans ON membership_plans.id = users.assigned_plan
                LEFT JOIN users up ON up.id = users.refid
                WHERE users.TYPE = "user" AND users.refid= '. $user_id;

            // $result = $this->db->query("SELECT id,username, CONCAT(firstname,' ',lastname) as name, avatar,refid FROM users WHERE type='user' AND refid={$user_id}")->result_array();
            ...
            $result = App\User::select('users.id', 'users.firstname','users.lastname','users.username','users.avatar','users.refid', 'users.Country')->with(['country', 'purchased_membership_plans.plan' => function($q) {
                $q->select('id', 'name');
            }])->where(['type' => 'user', 'refid' => $user_id, 'reg_approved' => 1])->get();
            ...
            foreach ($result as $key => $value) {
                ...
                $desc = '';
                $country_name = isset($value->country->name) ? $value->country->name : '';
                if($value->purchased_membership_plans->isNotEmpty())
                {
                    $plan_names = $value->purchased_membership_plans->pluck('plan.name')->unique()->toArray();
                    $desc = implode(',', $plan_names);
                }
                
                $desc = $desc . ((!empty($desc)) ? ', ' . $country_name : $country_name);
            ...
            return $users;
            // return $this->db->get_where('users', array('refid' => $user_id,'type' => 'user'))->select('id,username as name,avatar,refid')->result_array();
        }
        return false;
    }
}