...
function new_withdrwal_status($status){
    $label = '';
    switch ((int)$status) {
        case 0: $label = '<span class="badge badge-pill badge-info">Pending</span>'; break;
        case 1: $label = '<span class="badge badge-pill badge-success">Approved</span>'; break;
        case 2: $label = '<span class="badge badge-pill badge-danger">Rejected</span>'; break;
        default: $label = '<span class="badge badge-pill badge-warning">Unknow</span>'; break;
    }
...
function generateUserMembershipNumber() {
    $number = mt_rand(1000000000000000, 9999999999999999);

    if (userMembershipNumberExists($number)) {
        return generateUserMembershipNumber();
    }

    return $number;
}

function userMembershipNumberExists($number) {
    $CI =& get_instance();
    $check_membership_no = $CI->db->get_where('users',array('membership_no'=>$number))->row_array();
    // $check_membership_no = $CI->db->select('id,name')->from('users')->where('membership_no', $number)->get()->row_array();
    
    if (!empty($check_membership_no)) {
        return true;
    }
