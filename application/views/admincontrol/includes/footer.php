...
<footer class="footer common-side-footer">
	<div class="top-footer">
  		<span class="text-center"> <?= $SiteSetting['footer'] ?></span> <span class="common-footer-text">Made with </span><i class="mdi mdi-heart text-danger"></i><span class="common-footer-text"> By Bigg Innovate</span>  
	</div>
  	<div class="bottom-footer">
	   	<div  class="row align-items-center">
			<div class="col-6">
				<div  class="copyrights-content">
					<p class="m-0">Copyright 2021 | All Right Reserved</p>
				</div>
			</div>
			<div class="col-6">
				<ul class="list-inline text-sm-right static-page-links">
    ...
    <span class="text-center"><div title='Current Time : <?= date("Y-m-d h:i:s") . " " .date_default_timezone_get() ?>'></div></span>
...
<script type="text/javascript">
	...
	function handleUserPaymentDetailView(user_id, $this) {
		$.ajax({
			...
			success:function(json){
                ...
                html += '<th><?= __('admin.footer_mobile_no') ?></th>';

                html += '<td>'+ json.user.mobile_no +'</td>';

                html += '</tr>';

                html += '<tr>';

                html += '<th><?= __('admin.footer_dob') ?></th>';

                html += '<td>'+ json.user.dob +'</td>';

                html += '</tr>';

                html += '<tr>';

                html += '<th><?= __('admin.footer_nric') ?></th>';

                html += '<td>'+ json.user.nric_no +'</td>';

                html += '</tr>';

                html += '<tr>';

                html += '<th><?= __('admin.footer_billing_address') ?></th>';

                html += '<td>'+ json.user.billing_address +'</td>';
                ...
                html += '<th><?= __('admin.footer_membership_plan') ?></th>';

                html += '<td>'+ json.user.membership_plan +'</td>';

                html += '</tr>';

                html += '<tr>';

                html += '<th><?= __('admin.footer_total_referrals') ?></th>';

                html += '<td>' + json.user.total_referrals + '</td>';

                html += '</tr>';

                html += '<tr>';

                html += '<th><?= __('admin.footer_referred_by') ?></th>';

                html += '<td>'+ json.user.referred_by +'</td>';

                html += '</tr>';

                html += '<tr>';

                html += '<th><?= __('admin.footer_referral_mobile_no') ?></th>';

                html += '<td>'+ json.user.refnumber +'</td>';

                html += '</tr>';

                html += '<tr>';

                html += '<th><?= __('admin.footer_joined_date') ?></th>';

                html += '<td>'+ json.user.joined_date +'</td>';

                html += '</tr>';

                html += '<tr>';

                html += '<th><?= __('admin.footer_status') ?></th>';

                html += '<td>'+ json.user.status +'</td>';
	...
	$(document).delegate("[payment_detail]",'click',function(e){
		...
		handleUserPaymentDetailView(user_id, $this);
