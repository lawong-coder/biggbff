...
//return $this->db->query("ALTER TABLE users ADD google_id VARCHAR(255) NOT NULL, ADD facebook_id VARCHAR(255) NOT NULL, ADD twitter_id VARCHAR(255) NOT NULL");
	...
	function activateUserAndClearSession($user)
	{
		$user->uuid = NULL;
		...
		$this->session->unset_userdata('activate_user_data');

		$user_details_array = $this->login($user->username,true);
		$this->update_user_login($user_details_array['id']);
