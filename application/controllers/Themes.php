...
class Themes extends MY_Controller {
	...
	public function multiPages($ci, $childPage, $data = []){
		...
		$registration_builder['allow_back_to_login'] = true;
 		...
 		if($data['store']['registration_status']){
 			...
 			$registration_builder['edit_view'] = true;

			$registration_builder['allow_billing_address'] = true;

			$registration_builder['edit_view_refer'] = true;
			
			$registration_builder['is_fronted_form'] = isset($data['is_fronted_form']) ? $data['is_fronted_form'] : false;

			$registration_builder['refer_users'] = $ci->db->query("SELECT id,username FROM users WHERE id != ". (int)$id ." AND type='user'")->result_array();

			$registration_builder['plan_lists'] = $ci->db->query("SELECT id,name FROM membership_plans")->result_array();

			$registration_builder['countries'] = $ci->User_model->getCountries();

 			$data['register_fomm'] = $ci->load->view('auth/user/templates/register_form',$registration_builder, true);
 		}
 		if ($childPage) {
	 		$fixed_pages= ['register','register','login','contact','forget-password','terms-of-use','page', 'activate-user'];
 		...
 		if ($childPage) {
	 		switch ($childPage) {
	 			case 'activate-user':
	 				$ci->load->view('usercontrol/activate_user/index', $data);
	 			break;
				case 'page':
