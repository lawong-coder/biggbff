...
class Total_model extends MY_Model{
	public function adminTotals(){
		...
		$totals['admin_total_sale'] = (int)$this->db->query('SELECT SUM(total) as total FROM membership_user WHERE status_id=1')->row()->total;

		$totals['total_members'] = (int)$this->db->query('SELECT COUNT(id) as total FROM users WHERE type="user"')->row()->total;

		$totals['admin_total_commission'] = (int)$this->db->query('SELECT SUM(amount) as total FROM user_balance_logs WHERE type="referral_commission" AND status=1')->row()->total;

		$totals['admin_total_withdrawal'] = (int)$this->db->query('SELECT SUM(amount) as total FROM withdraw_requests WHERE status=1')->row()->total;

		$totals['admin_total_transfer'] = (int)$this->db->query('SELECT SUM(amount) as total FROM transfer_requests WHERE status=1')->row()->total;

		$totals['admin_total_franchise'] = (int)$this->db->query('SELECT COUNT(id) as total FROM membership_user WHERE status_id=1')->row()->total;
		...
		FROM order_products 
			LEFT JOIN `order` o ON o.id = order_products.order_id
		WHERE vendor_id > 0 AND o.status > 0 GROUP BY order_id ")->row();
	...
	public function chart($filter = []){
        ...
        foreach ($data as $key => $value) {
            $chart[$value['groupby']] = (int)$value['total_members'];
        }
        ...
        foreach ($defaultKey[$filter['group']] as $key => $value) {
        	if($value){
	        	$json['total_members'][$value] = isset($chart[$key]) ? $chart[$key] : 0;
        	}
        }
	...
	public function adminSalesChart($filter = []){
		$json = [];
		$orderBy = ' ORDER BY created_at DESC ';
        ...
        $this->db->order_by('created_at','DESC');
        $this->db->group_by($groupby);
        $data = $this->db->get('membership_user')->result_array();

        $chart = array();
        foreach ($data as $key => $value) {
            $chart[$value['groupby']] = $value['total_sale'];
        }
        ...
        foreach ($defaultKey[$filter['group']] as $key => $value) {
        	if($value){
	        	$json['total_sale'][$value] = isset($chart[$key]) ? $chart[$key] : 0;
        	}
        }
	...
	public function adminBalance($filter = []){
		...
		$wAccepted  = $this->db->query("SELECT SUM(amount) as total FROM wallet WHERE {$wallet_where} AND user_id > 1 AND status = 3")->row()->total;
		...
		$balance -= $wAccepted;
		$balance -= $this->db->query("SELECT SUM(amount) as total FROM wallet WHERE {$wallet_where} AND type IN('admin_transaction') AND status>0")->row()->total;
		...
		$balance -= $t= $this->db->query("
			...
			WHERE 
				wallet.type IN('sale_commission','welcome_bonus')
	...
	public function getUserBalance($user_id, $filter = []){
		...
		$admin_transaction = $this->db->query("SELECT SUM(amount) as total FROM wallet WHERE type='admin_transaction' AND user_id={$user_id} {$wallet_where} ")->row()->total;
	...
	public function getUserBiggCoin($user_id)
	{
		$total = App\MembershipPlan::whereHas('membership_users', function($query) use ($user_id) {
			$query->where('user_id', $user_id);
			$query->where('status_id', 1);
		})->where('status',1)->sum('bigg_coins');

		return $total;
	}

	public function getUserBiggCoupon($user_id)
	{
		$total = App\MembershipPlan::whereHas('membership_users', function($query) use ($user_id) {
			$query->where('user_id', $user_id);
			$query->where('status_id', 1);
		})->where('status',1)->sum('bigg_coupon');

		return $total;
	}

	public function getUserShareCredits($user_id)
	{
		$total = App\MembershipPlan::whereHas('membership_users', function($query) use ($user_id) {
			$query->where('user_id', $user_id);
			$query->where('status_id', 1);
		})->where('status',1)->sum('fego_shares');

		return $total;
	}

	public function getUserProductBalance($user_id)
	{
		$plan_products = App\MembershipPlan::whereHas('membership_users', function($query) use ($user_id) {
			$query->where('user_id', $user_id);
			$query->where('status_id', 1);
		})->where('status',1)->pluck('products', 'id');
		...
		if(count($plan_products) > 0)
		{
			foreach ($plan_products as $key => $plan_product) {
				if(!empty($plan_product))
				{
					$temp = json_decode($plan_product, 1);
					$products[] = $temp;

					$total += ((count($temp) > 0) ? array_sum(array_column($temp, 'quantity')) : 0);
	...
	public function calculateRefCommission($data, $sale_type, $referdata, &$balance = 0) {

       	foreach ($data as $key => $value) {
       		if(isset($value['id']))
       		{
       			$temp_total = App\MembershipUser::where('user_id', $value['id'])->where('status_id', 1)->sum('total');

       			$user_referlevel = (isset($value['user_referlevel'])) ? $value['user_referlevel'] : 0;
       			$user_referlevel = 'referlevel_'.$user_referlevel;

       			if(isset($referdata[$user_referlevel]) && isset($referdata[$user_referlevel]['sale_commition']) && !empty($referdata[$user_referlevel]['sale_commition']))
       			{
   					$commission = $referdata[$user_referlevel]['sale_commition'];

	       			$to_add = 0;
	       			if($sale_type == 'percentage')
	       				$to_add = (($temp_total * $commission) / 100);
	       			else
	       				$to_add = $commission;

	       			$balance += $to_add;

		            $t = $this->calculateRefCommission($value['children'], $sale_type, $referdata, $balance);
		        }
       		}
       	}
	...
	public function getUserWalletBalance($user)
	{
		...
		$levels = isset($data['referlevel']['levels']) ? (int)$data['referlevel']['levels'] : 0;
		$sale_type = isset($data['referlevel']['sale_type']) ? $data['referlevel']['sale_type'] : 'percentage';

		$total_balance = 0;

		if($levels > 0)
		{
			$refree_tree = $this->Product_model->getMyUnderLevelUsers($user->id);

			if(!empty($refree_tree))
			{
				$total_balance = $this->calculateRefCommission($refree_tree, $sale_type, $data);
			}
	...
	public function getRefChartDefaultKeys()
	{
		$week = [];
        $day = [];
        $year = [];
	...
	public function getTeamReferals($user_id)
	{
		$refids = App\User::where(['type' => 'user', 'refid' => $user_id])->pluck('id')->toArray();

		foreach ($refids as $key => $value) {
			$temp = App\User::where(['type' => 'user', 'refid' => $value])->pluck('id')->toArray();

			if(!empty($temp))
			{
				$refids = array_merge($refids, $temp);
			}
		}

		// $refids = array_merge($refids, [$user->id]);

		return $refids;
	}

	public function userRefChart($user, $filter = []){
		$json = [];
		$orderBy = ' ORDER BY created_at DESC ';
        ...
        $this->db->where($current_year);
        $this->db->where('type', 'user');
        $this->db->where('refid', $user->id);
        $this->db->where('reg_approved', 1);
        $this->db->order_by('created_at','DESC');
        $this->db->group_by($groupby);
        $data = $this->db->get('users')->result_array();
        ...
        $chart = array();
        foreach ($data as $key => $value) {
            $chart['direct'][$value['groupby']] = (int)$value['total_users'];
        }

        $refids = $this->getTeamReferals($user->id);
        ...
        foreach ($data as $key => $value) {
            $chart['team'][$value['groupby']] = (int)$value['total_users'];
        }

        $defaultKey = $this->getRefChartDefaultKeys();

        foreach ($defaultKey[$filter['group']] as $key => $value) {
        	if($value){
	        	$json['total_direct_refusers'][$value] = isset($chart['direct'][$key]) ? $chart['direct'][$key] : 0;
	        	$json['total_team_refusers'][$value] = isset($chart['team'][$key]) ? $chart['team'][$key] : 0;
        	}
        }
