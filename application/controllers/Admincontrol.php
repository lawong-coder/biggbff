...
class Admincontrol extends MY_Controller {
		...
		if ($this->input->post()) {
...

			{
				...
				$data=$this->user->insert(array(
					...
					'refid'     => !empty($refid) ? base64_decode($refid) : 1,
				...
				if(!empty($data)){
					...
					App\User::createDefaultSettings($id);
	...
	/*public function mywallet(){
		...
		$data['dashboard_referral_totals'] = $this->db->query("SELECT 
			(SELECT SUM(amount) as total FROM user_balance_logs WHERE created_at >= '".$year_ago."' AND status = 1 AND type = 'referral_commission') as year_ago_referral_fees,
			(SELECT SUM(amount) as total FROM user_balance_logs WHERE created_at >= '".$month_ago."' AND status = 1 AND type = 'referral_commission') as month_ago_referral_fees,
			(SELECT SUM(amount) as total FROM user_balance_logs WHERE created_at >= '".$week_ago."' AND status = 1 AND type = 'referral_commission') as week_ago_referral_fees,
			(SELECT SUM(amount) as total FROM user_balance_logs WHERE status = 1 AND type = 'referral_commission') as all_time_total_referral_fees
			FROM user_balance_logs")->row();
	...
	public function mywallet(){
		...
		$data['transaction'] = App\UserBalanceLog::where('type', 'referral_commission')->orderByDesc('created_at')->get();
	...
	public function ajax_dashboard(){
		...
		$data['sales_chart'] = $this->Total_model->adminSalesChart([

			'year' => $post['selectedsalesyear'],

			'group' => $post['renderSalesChart'],
		...
		$data['admin_totals']['admin_total_sale'] = c_format($data['admin_totals']['admin_total_sale']);
		$data['admin_totals']['admin_total_commission'] = c_format($data['admin_totals']['admin_total_commission']);
		$data['admin_totals']['admin_total_withdrawal'] = c_format($data['admin_totals']['admin_total_withdrawal']);
		$data['admin_totals']['admin_total_transfer'] = c_format($data['admin_totals']['admin_total_transfer']);
		$data['admin_totals']['admin_total_franchise'] = c_format($data['admin_totals']['admin_total_franchise']);
	...
	public function dashboard(){
		...
		$data['recent_users'] = App\User::where(['status' => 1])->orderByDesc('id')->take(10)->get();
		...
		if(isset($_GET['getSalesChartData'])) {
			$json['sales_chart'] = $this->Total_model->adminSalesChart($post);
			...
			if($this->form_validation->run()){
				...
				if(!isset($json['errors'])){
                    ...
                    if($id == 0){
                    	...
                    	$userArray['membership_no'] = generateUserMembershipNumber();
	...
	public function getPlan(){
		...
		$plan = $this->db->query('SELECT id, price FROM membership_plans WHERE id='.$plan_id)->row_array();
    ...
    public function getPlanPriceFormatted(){
		$plan_id = $this->input->post('plan_id',true);
		$plan = $this->db->query('SELECT id, price, hosting_cost FROM membership_plans WHERE id='.$plan_id)->row_array();
		$price = (isset($plan['price'])) ? c_format($plan['price']) : 0;
		$hosting_cost = (isset($plan['hosting_cost'])) ? c_format($plan['hosting_cost']) : 0;

        echo json_encode(['price' => $price, 'hosting_cost' => $hosting_cost]);
        die;
    }
	...
	public function addusers($id = null){
		...
		if ($this->input->post()) {
			...
			$this->form_validation->set_rules('nric_no', 'NRIC No', 'required');
			...
			if( count($json['errors']) == 0){
				...
				if(count($json['errors']) == 0){
                    ...
                    $userArray = array(
						...
						/*'reg_approved'				=> 0,*/
						'reg_approved'				=> (int)$this->input->post('reg_approved',true),
					...
					if(isset($post['membership_plan']))
					{
						$userArray['assigned_plan']	= $post['membership_plan'];
					}
                    ...
                    if((int)$id == 0){
						...
						if($id)
						{
                			...
                			if(isset($post['membership_plan']) && !empty($post['membership_plan']))
                			{
                				...
                				$plan = App\MembershipPlan::find($post['membership_plan']);

                				if($plan) {
			                		$plan->buy($user, 1, 'Default plan started by Admin','Default');
			            		}
		...
		$data['assigned_plans_price'] = $this->db->query('SELECT id, price FROM membership_plans')->result_array();
		...
		$data['allow_status_option'] = true;

		$data['allow_billing_address'] = true;
		...
		$data['plan_lists'] = $this->db->query("SELECT id,name FROM membership_plans")->result_array();
	...
	public function admin_pay_to_ewallet_tran() {

		$this->load->library('form_validation');

		$this->form_validation->set_rules('amount', 'Amount', 'required|trim|greater_than[0]');
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$html = '<h6 class="text-center"> Payment Confirmation </h6><hr>';

			$html .= '<p class="text-center"> Are you sure? </p>';

			$html .= "<br><div class='row'> <div class='col-sm-6'><button data-dismiss='modal' class='btn btn-primary btn-block'>No</button></div> <div class='col-sm-6'><button class='btn btn-success  btn-block' tran-amount-confirm='". $amount ."'>Yes</button></div> </div>";
	...
	public function add_transaction(){
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$adminTransaction = App\EwalletAdminTransaction::create([
			...
			if($adminTransaction)
			{
				App\EWallet::add_ewallet($adminTransaction->user_id, $adminTransaction->amount);

				$adminTransaction->userBalanceLog()->create([
                    'sender_id' => $userdetails['id'],
                    'receiver_id' => $adminTransaction->user_id,
                    'amount' => $adminTransaction->amount,
                    'date' => date('Y-m-d H:i:s'),
                    'commision' => NULL,
                    'commision_type' => NULL,
                    'type' => App\EwalletAdminTransaction::$balance_log_type,
                    'status' => 1
                ]);
	...
	public function getpaymentdetail($user_id)	{
        ...
        $customValue = (isset($user->value) && !empty($user->value)) ? json_decode($user->value, 1) : [];

        $my_franchise_plan = App\MembershipUser::with('plan')->where('user_id', $user->id)->where('status_id', 1)->orderBy('id')->select('id', 'plan_id', 'user_id')->get();

        $membership_plan = [];
        if(count($my_franchise_plan) > 0)
        {
        	foreach ($my_franchise_plan as $key => $franchise_plan) {
        		$membership_plan[] = ($franchise_plan->plan->name) ? $franchise_plan->plan->name : '';
        	}
        }

        $membership_plan = implode(', ', $membership_plan);

        $reffered_by = '';
        if (isset($user->refid) && !empty($user->refid))
		{
			$data = $this->db->query("SELECT username FROM users WHERE id='".$user->refid."'")->row_array();

			$reffered_by = (isset($data['username']) && !empty($data['username'])) ? $data['username'] : 'Admin';
		}

		$total_referrals = [];
		$refer_franchise_plan = App\MembershipUser::with('plan')->whereHas('user', function($query) use ($user) {
			$query->where('refid', $user->id);
		})->where('status_id', 1)->selectRaw('plan_id, count(*) as total')->groupBy('plan_id')->get();

		$exculed_plan = MembershipPlan::pluck('name', 'name')->toArray();
		
		if(count($refer_franchise_plan) > 0)
		{
			foreach ($refer_franchise_plan as $key => $refer_plan) {
				// $total_referrals[] = $refer_plan->plan->name . '(' . $refer_plan->total . ')';
				$total_referrals[$refer_plan->plan->name] = $refer_plan->total;
			}

			$exculed_plan = array_diff_key($exculed_plan, $total_referrals);

		}

		if(!empty($exculed_plan))
		{
			$exculed_plan = array_map(function($val) { return 0; }, $exculed_plan);
			$total_referrals = array_merge($total_referrals, $exculed_plan);
		}

		if(!empty($total_referrals))
		{
			$temp = [];
			foreach ($total_referrals as $key => $value) {
				$temp[] = $key . '(' . $value . ')';
			}

			$total_referrals = $temp;
		}

		// pred($total_referrals, $exculed_plan);
		$total_referrals = implode(', ', $total_referrals);
		...
		$data['user'] = array(
            ...
            'mobile_no'     => isset($customValue['custom_text-1611314653157']) ? $customValue['custom_text-1611314653157'] : '',
            
            'dob'     => isset($customValue['custom_date-1623412532756']) ? $customValue['custom_date-1623412532756'] : '',

            'nric_no'   => ($user->nric_no) ? $user->nric_no : '',
            
            'billing_address'   => ($user->billing_address) ? $user->billing_address : '',
            ...
            'refnumber' => ($user->refnumber) ? $user->refnumber : '',  

            'joined_date'      => $user->created_at,

            'status' => ($user->reg_approved == 1) ? 'Active' : 'Pending'
	...
	public function userslist(){
		...
		if ($this->input->post()) {
			...
			if(isset($post['action']) && $post['action'] == "process_approval") {
				...
				// $json['links'] = $this->build_paginate($userslist, 'admincontrol/userslist',$filter['page'], $filter['limit']);
				$total = $config['total_rows'];
				$found = count($data['userslist']);

				$result = "";
				if($total > 0) {
					$result = '<div><div style="color:#333">Showing<span style="color:#3E7CB3"> '. max((($filter['page']-1)*$filter['limit']),0) .'-'. ((($filter['page']-1)*$filter['limit'])+$found) .' </span>of <span style="color:#3E7CB3">'.$total.'</span> Results</div></div>';
				}

				$json['pagination_result'] = $result;
				$json['pagination'] = $this->pagination->create_links();
	...
	public function userslisttree(){
		...
		$data['userslist'] = $this->Product_model->getAllinOneNewFormatQuery(array(),0,true,true);
	...
	public function paymentsetting(){
		...
		$commonSetting = array('email','paymentsetting','integration','login', 'loginclient','productsetting','formsetting','tnc','site','affiliateprogramsetting','store','doc','googlerecaptcha','referlevel');
	...
	public function belts(){
		...
		$data['plans'] = MembershipPlan::all();

		$this->view($data,'belts/list');
	}

	public function belt($id){
		$this->load->model('PagebuilderModel');
		...
		if($id)
		{
			...
			$data['plan'] = $plan;
			$data['inqueue_users'] = App\MembershipUser::where('plan_id', $plan->id)->where('status_id', 1)->orderBy('created_at')->get()->unique('user_id');
			// $data['inqueue_users'] = $plan->inqueue_users->unique();

			$this->view($data,'belts/inqueue_users');
		}
		else
		{
			show_404();
	...
	public function withdrawal_requests_list(){
		...
		if (isset($post['delete_request'])) {
			...
			$req = $this->db->query("SELECT * FROM withdraw_requests WHERE id={$id}")->row();
			...
			if($req){
				...
				$this->db->query("DELETE FROM withdraw_requests WHERE id= {$id}");
			}
		...
		if (isset($post['get_new'])) {
			...
			$data['lists'] = $this->Withdraw_request_modal->getRequests($filter);



			$json['html'] = $this->load->view("admincontrol/withdrawal_requests/tr_w_request",$data,true);

			echo json_encode($json);die;

		}

		$data['Withdraw_request'] = App\WithdrawRequest::where('status', 0)->get();

		// $data['users'] = $this->db->query("SELECT id,username FROM users WHERE type = 'user'")->result_array();
		$data['users'] = $this->db->query("SELECT id,CONCAT(firstname,' ',lastname) as name FROM users ")->result_array();

		$this->view($data,'withdrawal_requests/index');
	}

	public function withdrawal_requests_details($id) {
		...
		// $data['request'] = $this->db->query("SELECT * FROM withdraw_requests WHERE id={$id}")->row_array();

		$withdrawRequest = App\WithdrawRequest::find($id);
		$data['request'] = $withdrawRequest;
		// pred($withdrawRequest);
		...
		if (isset($post['status'])) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$this->db->query("UPDATE withdraw_requests SET status=". (int)$post['status'] .", date='". date('Y-m-d H:i:s') ."', remarks='". $post['remarks'] ."' WHERE id={$id}");
				...
				else if($post['status'] == 2)
				{
					...
					App\EWallet::add_ewallet((int)$withdrawRequest->user_id, $withdrawRequest->amount);
				}

				if(isset($update_status))
				{
					$this->db->query("UPDATE user_balance_logs SET status=". $update_status .", is_locked=0 WHERE receiver_id=". (int)$withdrawRequest->user_id . " AND amount=". (int)$withdrawRequest->amount ." AND loggable_id=". $id . " AND is_locked=1");

					$withdrawRequest->refresh();
					$this->load->model('Mail_model');
					$this->Mail_model->send_ewallet_withdrwal_status_change($withdrawRequest);
				}
		...
		// $data['confirm'] = $this->Withdrawal_payment_model->getConfirm($withdrawRequest->withdraw_to,['request'=>$withdrawRequest);


		$this->view($data,'withdrawal_requests/details');
	...
	public function transfer_history(){
		...
		if (isset($post['get_new'])) {
			...
			$query = App\TransferRequest::where('status', 1);
			...
			if(!empty($filter))
			{
				...
				if (isset($filter['user_id'])) {
					$query = $query->where(function($q) use ($filter) {
						$q->where('user_id', (int)$filter['user_id']);
						$q->orWhere('receiver_id', (int)$filter['user_id']);
					});
				}
			}

			$data['lists'] = $query->with('receiver')->get();

			$json['html'] = $this->load->view("admincontrol/transfer_history/tr_request",$data,true);
		...
		$data['transfers'] = App\TransferRequest::with('receiver')->get();

		$this->view($data, 'transfer_history/index');
	}

	public function commission_paid($page = 1){
	    $get = $this->input->get(null,true);
		...
		$query = App\UserBalanceLog::where('type', 'referral_commission')->orderByDesc('id');

		if (isset($filter['user_id']) && $filter['user_id'] != '') {
			$query->where(function($q) use ($filter) {
				$q->where('receiver_id', (int)$filter['user_id']);
				// $q->orWhere('sender_id', (int)$filter['user_id']);
			});
		...
		$data['links'] = $this->build_paginate($query, 'admincontrol/commission_paid',$page, $limit);
		...
		$this->view($data, 'commission_paid/index');
